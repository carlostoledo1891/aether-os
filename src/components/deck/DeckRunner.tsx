...
import { ThemeBoundary } from '../../theme/ThemeProvider'
import { getThemeTokens, type ThemeId } from '../../theme/themeTokens'
  ...
  return (
    <ThemeBoundary theme={theme}>
      <div
        ...
        onTouchStart={onTS}
        onTouchEnd={onTE}
        style={{ width: '100vw', height: '100vh', position: 'relative', overflow: 'hidden', background: tokens.bg, color: tokens.text1, fontFamily: 'var(--font-sans)', userSelect: 'none' }}
      >

        <motion.div animate={{ opacity: [0.25, 0.45, 0.25] }} transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut' }}
          style={{ position: 'absolute', width: '50vw', height: '50vw', borderRadius: '50%', background: `radial-gradient(circle, ${accent}10, transparent 70%)`, top: '-10vw', left: '25vw', filter: 'blur(80px)', pointerEvents: 'none' }} />
        ...
        {idx > 0 && (
          <button type="button" onClick={e => { e.stopPropagation(); prev() }} aria-label="Previous slide"
            style={{ position: 'absolute', left: 20, top: '50%', transform: 'translateY(-50%)', zIndex: 10, background: tokens.glass06, border: `1px solid ${tokens.glass08}`, borderRadius: '50%', width: 40, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', color: tokens.text2, cursor: 'pointer', fontSize: 20 }}>‹</button>
        )}
        {idx < count - 1 && (
          <button type="button" onClick={e => { e.stopPropagation(); next() }} aria-label="Next slide"
            style={{ position: 'absolute', right: 20, top: '50%', transform: 'translateY(-50%)', zIndex: 10, background: tokens.glass06, border: `1px solid ${tokens.glass08}`, borderRadius: '50%', width: 40, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', color: tokens.text2, cursor: 'pointer', fontSize: 20 }}>›</button>
        )}
        ...
        <div style={{ position: 'absolute', bottom: 20, left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 6, zIndex: 10 }}>
          {Array.from({ length: count }, (_, i) => (
            <button key={i} type="button" aria-label={`Slide ${i + 1}`} onClick={e => { e.stopPropagation(); go(i) }}
              style={{ width: i === idx ? 24 : 6, height: 6, borderRadius: 3, border: 'none', background: i === idx ? accent : tokens.glass12, cursor: 'pointer', padding: 0, transition: 'width 0.3s ease, background 0.3s ease' }} />
          ))}
        </div>

        <div style={{ position: 'absolute', bottom: 20, right: 24, fontSize: 11, color: tokens.text4, fontFamily: 'var(--font-mono)', zIndex: 10 }}>
          {String(idx + 1).padStart(2, '0')} / {String(count).padStart(2, '0')}
        </div>

        <div style={{ position: 'absolute', bottom: 20, left: 24, fontSize: 10, color: tokens.text4, letterSpacing: '0.04em', zIndex: 10 }}>
          ← → navigate · ESC exit
        </div>
      </div>
    </ThemeBoundary>
...
function ReportRunner({ manifest, onClose }: { manifest: DeckManifest; onClose?: () => void }) {
  ...
  const content = (
    <>
      ...
      <motion.div
        ...
        style={{ ...lightboxStyle, background: tokens.bg, color: tokens.text1 }}
        role="dialog"
        aria-modal
  ...
  if (manifest.portal !== false) {
    return createPortal(<ThemeBoundary theme="light"><AnimatePresence>{content}</AnimatePresence></ThemeBoundary>, document.body)
  }

  return <ThemeBoundary theme="light"><AnimatePresence>{content}</AnimatePresence></ThemeBoundary>
}
