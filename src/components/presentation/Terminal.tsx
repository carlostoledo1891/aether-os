...
export function Terminal({ title, children, large }: TerminalProps) {
  ...
  return (
    <div style={{
      background: W.glass06, border: `1px solid ${W.glass08}`, borderRadius: 12,
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: dotGap }}>
          <div style={{ width: dotSize, height: dotSize, borderRadius: '50%', background: W.red, opacity: 0.7 }} />
          <div style={{ width: dotSize, height: dotSize, borderRadius: '50%', background: W.amber, opacity: 0.7 }} />
          <div style={{ width: dotSize, height: dotSize, borderRadius: '50%', background: W.green, opacity: 0.7 }} />
        </div>
        <span style={{ fontSize: titleFont, color: W.text3, fontFamily: 'var(--font-mono)', marginLeft: large ? 8 : 6 }}>{title}</span>
      ...
      <div style={{ padding: bodyPad, fontFamily: 'var(--font-mono)', fontSize: bodyFont, lineHeight: large ? 1.7 : 1.65, color: W.text2 }}>
        {children}
      </div>
    </div>
  )
