...
const LEADERS = [
  ...
  { who: 'Nick Tunks', role: 'Chairman', needs: 'One coherent field-to-filing story. The technical appendix is the leave-behind.', accent: V },
]
...
export function MeteorPlaySlide() {
  return (<>
    ...
    <p style={{ fontSize: 13, color: W.text3, marginBottom: 24, maxWidth: 640 }}>Wednesday April 16 — I'm demoing VeroChain live to Dr. de Carvalho, chief geologist at Poços de Caldas. Meteoric is the target first pilot, not yet contracted. This meeting is the gate. I hope you are in this with me!</p>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, width: '100%', maxWidth: 860 }}>
      {[
        { label: 'Target Ask', value: '$102k/yr' },
        { label: 'Tier', value: 'Growth' },
        { label: '% of Disclosed Revenue', value: '~0.03%' },
        { label: 'Target Integration', value: '~90 days' }
      ].map(s => (
        <div key={s.label} style={{ background: W.glass04, border: `1px solid ${W.glass06}`, borderRadius: 14, padding: '16px', textAlign: 'center' }}>
          <div style={{ fontSize: 11, color: W.text3, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 4 }}>{s.label}</div>
  ...
  return (<>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, maxWidth: 900, width: '100%', marginBottom: 32 }}>
      ...
      <div style={{ background: `${W.green}08`, border: `1px solid ${W.green}30`, borderRadius: 14, padding: '24px 20px', textAlign: 'center' }}>
        <div style={{ fontSize: 10, color: W.green, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Post-Pilot (Scenario)</div>
        <div style={{ fontSize: 28, fontWeight: 800, color: W.green, fontFamily: 'var(--font-mono)' }}>$5-7M</div>
        <div style={{ fontSize: 11, color: W.text3, marginTop: 6 }}>Modeled traction factor lift 3.5 → 4.5</div>
      </div>
      <div style={{ background: `${V}10`, border: `1px solid ${V}30`, borderRadius: 14, padding: '24px 20px', textAlign: 'center' }}>
        <div style={{ fontSize: 10, color: V, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Modeled Delta</div>
        <div style={{ fontSize: 28, fontWeight: 800, color: V, fontFamily: 'var(--font-mono)' }}>~$2M</div>
        <div style={{ fontSize: 11, color: W.text3, marginTop: 6 }}>Scorecard-implied valuation lift if pilot signs</div>
      </div>
    </div>
