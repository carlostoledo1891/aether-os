 ...
 * the Trust Center now leads with the public verifier URL. Compliance
 * mappings still matter, but they sit *under* a working cryptographic
 * proof — the verifier is the floor, the mappings are the ceiling.
 */
const REFERENCE_VERIFIER_PATH = '/verify/c1a32f57bf64b88c845166007e16f12a3522589dcc4f90cad572f14ba8512d1a'

export function TrustHeroSlide() {
  return (
    <MarketingSlideRoot>
      <header style={{ minHeight: '55vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '48px 16px 32px', position: 'relative', overflow: 'hidden' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease }}>
          ...
          <h1 style={{ fontSize: 'clamp(32px, 4.5vw, 56px)', fontWeight: 800, lineHeight: 1.08, marginBottom: 20, maxWidth: 800 }}>
            Don&apos;t take our word for it.
          </h1>
          <p style={{ ...body, margin: '0 auto 24px', textAlign: 'center', maxWidth: 660 }}>
            The reference evidence bundle below recomputes every SHA-256 in its audit chain inside your tab, using <code style={{ fontFamily: 'var(--font-mono)' }}>crypto.subtle</code>. The server never gets to be the one that says &quot;valid&quot;. Open the link, watch the verdict resolve, then read the control mappings below if you still want them.
          </p>
          <a
            href={REFERENCE_VERIFIER_PATH}
            data-testid="trust-hero-verifier-cta"
            style={{
              display: 'inline-flex',
              alignItems: 'center',
              gap: 10,
              ...
              marginBottom: 32,
              boxShadow: `0 0 24px ${W.green}40`,
            }}
          >
            Verify the reference bundle in your tab →
          </a>
        </motion.div>
...
export function TrustArchitectureSlide() {
  return (
    <MarketingSlideRoot>
        ...
        <div style={wrap}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16, marginBottom: 24 }}>
            {[
              ...
              { title: 'Data Honesty', items: ['Provenance tags on every data point', 'Disclosure mode for IR / board safety', 'Mock / live boundary enforced', 'TTL=0 on regulated / sensitive fields'] },
            ].map((c, i) => (
...
export function TrustComplianceSlide() {
  return (
    <MarketingSlideRoot>
      <S id="compliance" style={{ padding: '32px 0' }}>
        <div style={wrap}>
          <SectionHeader
...
export function TrustNistSlide() {
  return (
    <MarketingSlideRoot>
      <S id="nist" style={{ padding: '32px 0' }}>
        <div style={wrap}>
          <SectionHeader
...
export function TrustSbomSlide() {
  return (
    <MarketingSlideRoot>
      <S id="sbom" style={{ padding: '32px 0' }}>
        <div style={wrap}>
          <SectionHeader
...
export function TrustDisclosureSlide() {
  return (
    <MarketingSlideRoot>
      <S id="disclosure" style={{ padding: '32px 0' }}>
        <div style={wrap}>
          <SectionHeader
...
export function TrustCtaSlide() {
  return (
    <MarketingSlideRoot>
      <S style={{ padding: '40px 0 24px', textAlign: 'center' }}>
        <div style={wrap}>
          <h2 style={{ ...heading, marginBottom: 12 }}>Questions About Security?</h2>
          <p style={{ ...body, margin: '0 auto 28px', textAlign: 'center' }}>
            Request a security briefing — we'll walk through our control mappings, evidence chain, and disclosure posture on a live call.
          </p>
      ...
      <footer style={{ padding: '24px 16px', textAlign: 'center', borderTop: `1px solid ${W.glass06}` }}>
        <p style={{ color: W.text4, fontSize: 11, margin: 0, lineHeight: 1.5, maxWidth: 600, marginInline: 'auto' }}>
          © 2026 Vero. Controls mapped to NIST 800-53 Rev 5 and CMMC Level 2. FedRAMP authorization pending cloud partner selection.
