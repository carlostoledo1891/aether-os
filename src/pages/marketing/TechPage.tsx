  ...
  return (
    <div style={{ background: W.bg, color: W.text1, fontFamily: 'var(--font-sans)', minHeight: '100vh' }}>
      ...
      <S style={{ padding: '80px 0' }}>
        <div style={wrap}>
          <SectionHeader
            ...
            body={<>All data flows through <code style={{ color: V, fontFamily: 'var(--font-mono)' }}>AetherDataService</code> — a TypeScript interface with 40+ methods. Components never know if they're consuming mock data or live API responses.</>}
          />
      ...
      <S id="dpp" style={{ padding: '80px 0' }}>
        <div style={wrap}>
          <SectionHeader
            label="DPP &amp; Blockchain Pipeline"
            heading="Compliance-Ready. Blockchain-Ready."
            body="Digital Product Passport export aligned to CEN/CENELEC standards. SHA-256 append-only audit chain provides immutable evidence of every data event. The pipeline to full blockchain attestation is architected — we own more of the process with each release."
          />
      ...
      <S id="data" style={{ padding: '80px 0' }}>
        <div style={wrap}>
          <SectionHeader
            label="Data Layer"
            heading="19 GeoJSON Datasets. Schema-Tested."
            body="Geological, hydrological, and infrastructure data loaded from typed GeoJSON with schema validation. JORC-safe resource classification with explicit methodology labeling. Geology and hydrology kept in separate visual layers — the digital twin never pretends to prove the deposit."
          />
        ...
        <div style={wrap}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 32 }}>
            {[
              ...
              { title: 'AGENT.md', desc: 'AI-ready bootstrap doc. Architecture map, run instructions, documentation index. Developer or agent onboarding in minutes.' },
      ...
      <S id="roadmap" style={{ padding: '80px 0' }}>
        <div style={wrap}>
          <SectionHeader
            ...
            heading="Integration Evolution"
            body="From LAPOC instruments to full blockchain anchoring — the technical path from pilot to enterprise."
          />
