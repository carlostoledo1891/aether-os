...
|------------|------------------|----------------|--------------|-----|------------|-------|
| `ops_reality_master` | Mixed | User-authored terrain master FeatureCollection (licences, plants, drills, APA, pit, dump, alkaline complex) | 2026-04-07 | EPSG:4326 | georeferenced_figure | Split into thematic GeoJSON in `src/data/geojson/`; retained for authored overlays but no longer the source for the alkaline complex boundary. |
| `anm_sigmine_geociencias_pocos_caldas_complexo_alcalino` | Polygon | ANM SIGMINE geociências ArcGIS layer `2` (`Poços de Caldas`, `Complexo alcalino`, `SGUnidade=K2_lambda_pc`) extracted via REST query and committed to `src/data/geojson/caldeira-boundary.geojson` | 2026-04-15 | EPSG:4326 | api_snapshot | Replaces the prior placeholder circular boundary. Source endpoint: `https://geo.anm.gov.br/arcgis/rest/services/SIGMINE/geociencias/MapServer/2/query`. Current extract contains 3 polygons (`OBJECTID` 237387, 269853, 269886). |
...
| `drillholes_caldeira` | Point | ASX **02766588** Table 2 + **02909601** Appendices 1–2 + `dd_intercept_highlights.json` + additive `02766588` Table 1 assay lookup | 2026-04-09 | EPSG:4326 | survey_appendix | Collars: SIRGAS 2000 / UTM 23S (EPSG:31983) via `proj4`; metadata + `easting_utm` / `northing_utm` on each feature. Builder preserves committed additive fields such as `hmreo_ppm`, `prnd_ppm`, and `lithology_intervals` during rebuilds. |
...
| `target_license_manifest` | JSON manifest | [`data/caldeira/target-licenses.json`](../../../data/caldeira/target-licenses.json) — curated mapping between internal `LIC-*` target areas and selected SIGMINE `process_number` values. | 2026-04-15 | n/a | curated_mapping | Versioned business list intended to change over time without rewriting map components. |
| `sigmine_target_processos_curated` | Polygon | Derived subset from [`target-licenses.json`](../../../data/caldeira/target-licenses.json) filtered against `src/data/geojson/external/caldeira-sigmine-tenements.geojson`; output committed to `src/data/geojson/external/caldeira-sigmine-target-tenements.geojson`. | 2026-04-15 | EPSG:4326 | curated_snapshot | Built with `npm run build:caldeira-sigmine-targets`. Preserves the full SIGMINE layer and exposes a separate target-only highlight layer. |
...
2. New ANM shapefile → replace `caldeira-licenses.geojson` or add a new manifest-backed tenement layer and register it through `shared/sites/caldeiraLayers.ts`.
3. New FEAM/COPAM map → re-georeference APA/pits/plant; bump `retrieved_on` and `confidence` here.
4. New ASX collar / intercept appendix → extend [`data/caldeira/staging/`](../../../data/caldeira/staging/) → `npm run build:caldeira-geojson` → if the explicit assay lookup changed, run `npm run backfill:drill-assays` → commit GeoJSON + bump rows in this table (`retrieved_on`).
...
8. Target-license highlight refresh → update `data/caldeira/target-licenses.json`, run `npm run build:caldeira-sigmine-targets`, then spot-check the derived subset against `caldeira-licenses.geojson` before committing.
