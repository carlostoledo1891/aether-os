...
## Aggregate Scorecard — v15 (Post Online Report Templates, 2026-04-10)

| Persona | v1 | v2 | v3 | v4 | v5 | v6 | v7 | v8 | v9 | v10 | v10.1 | v11 | v12 | v13 | v15 | Delta (v13→v15) | Biggest remaining gap |
|---------|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|-------|-----|-----|-----|-----|------------------|-----------------------|
| Chairman (Tunks) | 7.5 | 8.0 | 8.0 | 8.0 | 8.5 | 8.5 | 8.5 | 9.0 | 9.5 | 10.0 | 10.0 | 10.0 | 10.0 | 10.0 | **10.0** | — | Code ceiling reached |
| CEO (Gale) | 7.0 | 7.5 | 7.5 | 7.5 | 7.5 | 8.0 | 8.0 | 8.5 | 9.0 | 9.5 | 9.5 | 10.0 | 10.0 | 10.0 | **10.0** | — | Code ceiling reached |
| Chief Geologist | 7.5 | 7.5 | 7.5 | 7.5 | 8.0 | 8.5 | 8.5 | 8.5 | 9.0 | 9.5 | 9.5 | 10.0 | 10.0 | 10.0 | **10.0** | — | Code ceiling reached |
| DoD | 6.5 | 7.0 | 7.0 | 7.0 | 7.0 | 7.0 | 7.0 | 7.0 | 7.5 | 7.5 | 7.5 | 7.5 | 7.5 | 8.0 | **8.0** | — | FedRAMP certification |
| EU Enforcement | 6.0 | 6.5 | 6.5 | 6.5 | 7.5 | 7.5 | 7.5 | 7.5 | 8.0 | 8.5 | 8.5 | 8.5 | 8.5 | 8.5 | **8.5** | — | 100% DPP field coverage |
| PF Analyst | 7.0 | 7.5 | 7.5 | 7.5 | 7.5 | 7.5 | 7.5 | 8.0 | 8.0 | 8.0 | 8.0 | 8.5 | 8.5 | 9.0 | **9.5** | +0.5 | Covenant monitoring |
| NGO | 5.5 | 6.0 | 6.0 | 6.0 | 7.0 | 7.0 | 7.0 | 7.0 | 7.5 | 8.0 | 8.0 | 8.0 | 8.0 | 8.0 | **8.5** | +0.5 | Field-verified springs |
| SCADA | 7.5 | 8.5 | 8.5 | 8.5 | 9.0 | 9.0 | 9.0 | 9.0 | 9.0 | 9.5 | 9.5 | 10.0 | 10.0 | 10.0 | **10.0** | — | Code ceiling reached |
| Journalist | 6.5 | 7.0 | 7.0 | 7.0 | 7.5 | 8.0 | 8.0 | 8.5 | 9.0 | 9.5 | 9.5 | 10.0 | 10.0 | 10.0 | **10.0** | — | Code ceiling reached |
| **Weighted avg** | **6.8** | **7.3** | **7.3** | **7.3** | **~7.8** | **~8.0** | **~8.0** | **~8.4** | **~8.6** | **~8.9** | **~8.9** | **~9.2** | **~9.2** | **~9.3** | **~9.4** | +0.1 | |

---

## v15 Synthesis

The weighted average advances to ~9.4. Two personas moved on the report templates feature — a deliverable that transforms Vero from "dashboard tool" to "reporting platform."

**What this sprint delivered:**
1. **3 interactive report templates** — Environment, Operations, Drill Tests — each a rich, vertically-scrollable light-mode page with data visualizations, tables, progress bars, and timeline components.
2. **Light-mode palette (`WL`)** — a complete mirror of the dark `W` token system, purpose-built for print-friendly report rendering.
3. **PDF export** — `window.print()` with `@media print` CSS hides chrome and flattens the lightbox. Zero new dependencies.
4. **Time range selector** — 7d / 30d / 90d / 1yr / All presets on every report. Ready to wire to live data when available.
5. **ViewSwitcher dropdown** — hover-activated "Reports" button after Executive Overview tab. Dropdown presents 3 report options. Opens in a portal lightbox (z-index 300+) without disrupting dashboard state.

**Why two scores moved:**
- **PF Analyst (9.0 → 9.5):** The Operations Report is exactly what a project finance analyst wants to see during due diligence — NPV by scenario, CAPEX breakdown by category, C1/AISC cost bars, process flow, and pilot plant recovery data, all in a single exportable page. The time range selector signals data pipeline maturity. The PDF export means they can attach it to a credit committee memo.
- **NGO (8.0 → 8.5):** The Environment Report directly addresses the NGO's core concern — water quality monitoring with threshold bars, spring distribution by health status, APA/buffer zone compliance, community acceptance rates, sustainability design (dry stack tailings, closed-loop water, renewable grid). Having this as a standalone, light-mode, exportable page — separate from the mining dashboard — signals that environmental data is not hidden inside operational views.

**Why 7 scores held:** 5 personas remain at code ceiling (10.0). DoD and EU Enforcement gaps (FedRAMP, DPP field coverage) require external certifications, not UI features.

**Business Expert's verdict:** This is the deliverable that turns a "look at our dashboard" demo into "here's your first report." The lightbox-over-dashboard architecture is smart — it means the report inherits all the data context without forcing a page reload. The PDF export via `window.print()` is pragmatically correct for this stage: zero dependencies, works immediately, and the light theme ensures readable output. When Stuart Gale asks "can I send this to my board?", the answer is now "File → Print → Save as PDF."

**CTO's verdict:** Seven new files, four modified, zero new dependencies, zero test regressions. The `WL` palette as a separate constant rather than a CSS theme toggle was the right call — it avoids polluting the main theme system while delivering exactly what's needed. The portal architecture preserves dashboard state. The `@media print` approach is simple and robust. Next priorities: wire time range to actual data queries when live mode is active, add embedded maps to the reports (currently data-only), consider Puppeteer for server-side PDF generation if pixel-perfect output becomes a requirement.
